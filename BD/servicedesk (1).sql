...
SELECT s.idServicios,s.fecha,s.fecha_fin,s.horaInicio,s.horaFin,s.tipoServicio,s.descripcion,t.nombre,s.finalizado FROM servicios s INNER JOIN serviciostecnicos st INNER JOIN tecnicos t ON s.idServicios = st.fk_idServicios AND t.idTecnicos = st.fk_idTecnicos WHERE 1 ORDER BY s.idServicios;

CREATE DEFINER=`root@localhost` PROCEDURE `SelectServiciosNoFinalizados` ()  NO SQL
SELECT s.idServicios,s.fecha,s.fecha_fin,s.horaInicio,s.horaFin,s.tipoServicio,s.descripcion,s.finalizado,a.nombre FROM servicios s INNER JOIN areas a INNER JOIN servicioarea sa ON s.idServicios = sa.fk_idServicio AND a.idArea = sa.fk_idArea WHERE s.finalizado = 0 ORDER BY s.idServicios;
...
SELECT s.idServicios,s.fecha,s.horaInicio,s.horaFin,s.tipoServicio,s.descripcion,s.finalizado FROM servicios s INNER JOIN serviciosusuarios su INNER JOIN usuarios u ON su.fk_idServicios = s.idServicios AND su.fk_idUsuarios = u.idUsuarios WHERE u.idUsuarios = idUser AND s.finalizado = 0 AND s.mostrar = true;
